...
PrintingTip TipsManager::get_random_unique_tip() {
  ...
  for (const auto& tip : tips_cache) {
    ...
    if (std::find(viewed_tip_ids_.begin(), viewed_tip_ids_.end(), tip.id) == viewed_tip_ids_.end()) {
  ...
  spdlog::debug("[TipsManager] Selected unique tip '{}' ({}/{})",
                selected.id, viewed_tip_ids_.size(), tips_cache.size());
